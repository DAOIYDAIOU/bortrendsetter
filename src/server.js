...
app.post('/api/public/order', async (req, res) => {
  ...
  const order = await prisma.order.create({
    data: {
      ...
      fullName: customer?.fullName || `${user.first_name || ''} ${user.last_name || ''}`.trim() || null,
  ...
  const settings = await prisma.storeSetting.findUnique({ where: { id: 1 } });
  const currency = settings?.currency || config.defaultCurrency;
  const lines = order.items.map((item) => `• ${item.title}${item.size ? ` [${item.size}]` : ''} × ${item.quantity} — ${money(item.price * item.quantity, currency)}`);
  const message = [
    '🛒 Новый заказ',
    `#${order.id}`,
